    ...
    def __init__(self,location):
        ...
        while True:
            ...
            if self.q == 0:
                ...
                try:
                    while True:
                        self.dist.append(float(math.sqrt((data[i][5] - self.lat)**2 + (data[i][6] - self.long)**2))) #create list of distances
                        self.num.append(i)
                        i+=1
                except:
                ...
                if self.dist[0] < 5:
                    ...
                    print("Please try again lol") #Request for another input if location is too far
                    location = input("\nEnter Location(Type exit to stop)\nInput:")
            else:
                break
